$(document).ready(function () {
  ...
  function Grilla() {
      // armarJSON(1);
      $.ajax({
          url: '../../validaciones/ministerio/mostrarMinisterio.php',
          data: 1,
          type: 'POST',
          success: function (resp) {
              ...
              $.each(resp, function (indice, item) {
                  $("#tblNac").append($('<tr>').append($('<td>' + item.id + '</td>' +
                          ...
                          '<td><button type="button" class="btn btn-outline btn-success btn-sm" data-toggle="modal" data-target="#myModal" onclick="modificar('+item.id+','+"'"+item.descripcion+"'"+')">Modificar</button>'+' '+'<button type="button" class="btn btn-outline btn-danger btn-sm" onclick="eliminar('+item.id+')">Eliminar</button></td>'
                        )));
              });
          },
          error: function () {
  ...
  function Guardar() {
    ...
    $.ajax({
        url: '../../validaciones/ministerio/registroMinisterio.php',
        data: datosJSON,
        type: 'POST',
        success: function (resp) {
  ...
  function buscarDescripcion() {
    $('#buscar').keyup(function () {
      ...
      $('#cargando').show();
      $.ajax({
          url: '../../validaciones/ministerio/buscarMinisterio.php',
          data: datos,
          type: 'POST',
          success: function (resp) {
             // alert(resp);
             ...
             $.each(resp, function (indice, item) {
                 $("#tblBusqueda").append($('<tr>').append($('<td>' + item.id + '</td>' +
                         '<td>' + item.descripcion + '</td>'+
                         '<td><button type="button" class="btn btn-outline btn-success btn-sm" data-toggle="modal" data-target="#myModal" onclick="modificar('+item.id+','+"'"+item.descripcion+"'"+')" data-dismiss="modal">Modificar</button>'+' '+'<button type="button" class="btn btn-outline btn-danger btn-sm" onclick="eliminar('+item.id+')">Eliminar</button></td>'
                       )));
             });
             $('#cargando').show();
